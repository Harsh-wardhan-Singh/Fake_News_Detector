...
if not DATA_FILE.exists():
    print("ERROR: Big_training_data.csv not found!")
    exit()
...
if not required_cols.issubset(df.columns):
    print("ERROR: Big_training_data.csv must contain columns: text, label")
    exit()
...
try:
    ...
    print("\nWARNING: Stratify failed. Using normal split.")
    x_train, x_test, y_train, y_test = train_test_split(
        df["text"], df["label"],
        test_size=TEST_SIZE,
        random_state=RANDOM_STATE
...
# TF-IDF optimized for headlines
...
print("[STEP 2/3] Training LinearSVC model...")

base_model = LinearSVC(class_weight="balanced")

# Calibration makes probabilities possible
